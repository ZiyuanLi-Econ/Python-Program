...
share_table["Share of maritime trade (%)"] = (
    share_table["Hormuz flow (mb/d)"] / share_table["World maritime trade (mb/d)"] * 100
)
share_table["Share of total supply (%)"] = (
    share_table["Hormuz flow (mb/d)"] / share_table["World total supply (mb/d)"] * 100
)
...
colors = [
    "#C65D3A" if name == "Strait of Hormuz" else "#5B7FA3"
    for name in physical_routes["Location"]
]
...
ax.bar_label(
    bars,
    labels=[f"{value:.1f}" for value in physical_routes["2024"]],
    padding=4,
)
